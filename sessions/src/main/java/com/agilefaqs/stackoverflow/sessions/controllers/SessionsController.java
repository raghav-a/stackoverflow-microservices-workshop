...
public class SessionsController {
    ...
    @RequestMapping(value = "/authenticateToken", method = RequestMethod.GET)
    public UserDetail authenticateToken(@RequestHeader("X-AUTH-TOKEN") String authToken) {

        log.info("AuthRequest received : {}",authToken);
        final UserDetail response = sessionsService.validateToken(authToken);
        log.info("Validate token response is : "+response);
        return response;
    }
